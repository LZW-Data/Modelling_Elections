...
to setup
  ...
  set candidate-policies [ [ 0 0 0 0 0 ] [ 0 1 0 1 0 ] [ 1 0 0 0 1 ] [ 1 0 1 0 1 ] [ 1 1 1 1 1 ] ]
...
to go
  ;; update a number of patches at a time
  ask patches [ update ]
  tick  ;; updating patches and plots

  let poll [ 0 0 0 0 0 ]        ;; total number of voters voting (1, 2, 3, 4, 5) respectively
  let j 0
  ;; poll
  ask patches [ 
      while [ j < 5 ] [
        if vote = ( j + 1 ) [ set poll replace-item j poll ( item j poll + 1 ) ]
        set j j + 1
      ]
  ]


  let ranking [ 0 0 0 0 0 ]     ;; highest ranking candidate from left to right
  ...
  while [ n < 5 ] [      ;; loop through all candidates
    while [ j < 5 ] [    ;; loop to see how many other candidates are ahead in the poll
      if ( item n poll ) < ( item j poll ) [ set C ( C + 1 ) ]
      if ( item n poll ) = ( item j poll ) [ set test test + 1 ]
      set j ( j + 1 )
    ]
    ifelse test = 0 [ set ranking replace-item C ranking n ] [  ]  ;; placeholder for future removal of bias
    set n ( n + 1 )
  ]

  set j 0 ;; reset to be used in next procedure

  ;; update candidate policies based on poll rankings
  while [ j < 5 ] [                                                 ;; first loop goes through the rankings
    set n 0
    let index1 item j ranking
    let index2 0
    let loopcondition [ 1 1 1 1 1 ]                                 ;; list displaying which indices haven't been occupied yet
    while [ n < j ] [                                               ;; changes different number of policies per ranking
      while [ ( item index2 loopcondition ) = 0 ] [                 ;; stop (random 5) from choosing the same index each time
        set index2 ( random 5 )
      ]
      set loopcondition replace-item index2 loopcondition 0         ;; tell list that this index has been used

      set candidate-policies replace-item index1 candidate-policies ( replace-item index2 ( item index1 candidate-policies ) abs( ( item index2 ( item index1 candidate-policies ) ) - 1 ) )
      set n n + 1
    ]
    set j j + 1
  ]
...
to update  ;; patch procedure
  ...
  ask neighbors [
      while [ k < 5 ] [
        if vote = ( k + 1 ) [ set total replace-item k total ( item k total + 1 ) ]
        set k k + 1
      ]
  ]
